import{_ as g,O as b,R as y,A as m,T as d,i as r,C as S,H as P,g as R}from"./index-Ct-ulYoa.js";class T{constructor(){this.isHost=!1,this.mySessionId="",this.isGameStarting=!1,this.isManuallyLeaving=!1,this.isPageUnloading=()=>!1,this.countdownInterval=null,this.waitingUI=null,this.playerGridEl=null,this.playerCountEl=null,this.startBtn=null,this.waitingMsg=null,this.nameInput=null,this.roomListEl=null,this.hostIndicatorEl=null,this.backBtn=null,this.characterPopup=null,this.characterPreviewEl=null,this.waitingSpawnerInterval=null}async init(t){t.room&&(this.room=t.room,this.mySessionId=this.room.sessionId),this.isHost=t.isHost!==void 0?t.isHost:!1;let e=!1;if(window.addEventListener("beforeunload",()=>{e=!0}),this.isPageUnloading=()=>e,t.isRestore&&!this.room&&t.client&&await this.restoreRoom(t.client),!this.room){console.error("[PlayerWaitingRoomManager] Failed to initialize/restore room. Aborting start.");return}this.start()}async restoreRoom(t){const e=localStorage.getItem("currentReconnectionToken");if(!e){console.warn("Cannot restore player room: No reconnection token saved."),this.cleanupAndGoLobby();return}try{console.log("Player reconnecting with token..."),this.room=await t.reconnect(e),console.log("Player reconnected!",this.room),this.mySessionId=this.room.sessionId,localStorage.setItem("currentReconnectionToken",this.room.reconnectionToken),this.setupRoomListeners()}catch(a){console.warn("Player reconnection failed, attempting fallback join:",a);const i=localStorage.getItem("currentRoomId");if(i)try{console.log(`[PlayerLobby] 🔄 Attempting fresh join to room: ${i}`);const o=JSON.parse(localStorage.getItem("game_user_profile")||"{}"),n=o.nickname||o.fullname||o.username||"Player",s=localStorage.getItem("player_hair_id");this.room=await t.joinById(i,{name:n,userId:o.id,avatarUrl:o.avatar_url||"",hairId:s?parseInt(s):void 0}),console.log("[PlayerLobby] ✅ Re-joined successfully via fallback!"),this.mySessionId=this.room.sessionId,localStorage.setItem("currentReconnectionToken",this.room.reconnectionToken),this.setupRoomListeners();return}catch(o){console.error("[PlayerLobby] ❌ Fallback join also failed:",o)}localStorage.removeItem("currentRoomId"),localStorage.removeItem("currentSessionId"),localStorage.removeItem("currentReconnectionToken"),this.cleanupAndGoLobby()}}startGameEngine(t,e){g(()=>import("./index-Ct-ulYoa.js").then(a=>a.e),[]).then(a=>{a.initializeGame(t,e)}).catch(a=>{console.error("Failed to load game engine:",a),window.location.href="/"})}startManager(t,e){t==="LobbyManager"&&g(()=>import("./index-Ct-ulYoa.js").then(a=>a.p),[]).then(a=>{new a.LobbyManager().init(e)})}cleanupAndGoLobby(){var e;this.waitingUI&&this.waitingUI.classList.add("hidden");const t=document.getElementById("waiting-ui");t&&t.classList.add("hidden"),(e=document.getElementById("exit-confirm-modal"))==null||e.remove(),b.disable(),y.navigate("/"),this.startManager("LobbyManager")}setupRoomListeners(){this.room.state.listen("hostId",t=>{t===this.mySessionId&&this.startGameEngine("HostWaitingRoomScene",{room:this.room,isHost:!0}),this.updateUILayout()}),this.room.state.listen("isMusicEnabled",t=>{console.log(`[PlayerLobby] 🏠 Room Music Enabled: ${t}`),m.getInstance().setRoomMute(!t)}),this.room.state.isMusicEnabled!==void 0&&m.getInstance().setRoomMute(!this.room.state.isMusicEnabled),this.room.state.players.onAdd((t,e)=>{this.updateAll(),t.listen("name",()=>this.updateAll()),t.listen("hairId",()=>this.updateAll())}),this.room.state.players.onRemove(()=>this.updateAll()),this.room.onLeave(async t=>{if(console.log(`[PlayerLobby] Room connection lost (code: ${t}). isManuallyLeaving: ${this.isManuallyLeaving}`),!(t===1e3||this.isManuallyLeaving||this.isPageUnloading()||this.isGameStarting)){d.showWaiting(r.t("game.reconnecting")||"Koneksi terputus, menghubungkan kembali...",0);try{const e=window.colyseusClient,a=localStorage.getItem("currentReconnectionToken");if(e&&a){console.log("[PlayerLobby] Attempting to reconnect...");const i=await e.reconnect(a);console.log("[PlayerLobby] Reconnected successfully!"),this.room=i,localStorage.setItem("currentReconnectionToken",i.reconnectionToken),this.setupRoomListeners(),this.updateAll(),this.updateUILayout(),d.close(()=>{});return}}catch(e){console.error("[PlayerLobby] Reconnection failed:",e)}this.cleanupAndGoLobby()}}),this.room.onMessage("gameStarted",()=>{this.handleGameStart()}),this.room.onMessage("kicked",t=>{this.leaveRoom()}),this.room.onMessage("hostLeft",()=>{this.showHostLeftModal()}),this.room.onMessage("playerLeft",()=>{this.updateAll()})}start(){document.title="Waiting Room | Zigma",window.addEventListener("languageChanged",()=>{this.backBtn&&(this.backBtn.innerText=r.t("player_lobby.exit"));const a=document.getElementById("player-choose-char-btn");a&&(a.innerText=r.t("player_lobby.choose_character"));const i=document.getElementById("exit-confirm-title");i&&(i.innerHTML=r.t("player_lobby.dialog_exit_title"));const o=document.getElementById("exit-confirm-desc");o&&(o.innerHTML=r.t("player_lobby.dialog_exit_desc"));const n=document.getElementById("exit-cancel-btn");n&&(n.innerText=r.t("player_lobby.dialog_exit_cancel"));const s=document.getElementById("exit-confirm-btn");s&&(s.innerText=r.t("player_lobby.dialog_exit_confirm")),this.updatePlayerGrid()}),this.waitingUI=document.getElementById("waiting-ui");const t=document.getElementById("lobby-ui");t&&t.classList.add("hidden"),this.waitingUI&&(this.waitingUI.classList.remove("hidden"),this.setupPlayerUI()),this.playerGridEl=document.getElementById("player-grid"),this.playerCountEl=document.getElementById("player-count-value"),this.nameInput=document.getElementById("header-player-name"),this.backBtn=document.getElementById("player-back-btn"),this.backBtn&&(this.backBtn.onclick=()=>{this.showExitConfirm()});const e=document.getElementById("player-choose-char-btn");if(e&&(e.onclick=()=>{var i;const a=this.room.state.players.get(this.mySessionId);(i=this.characterPopup)==null||i.show((a==null?void 0:a.hairId)||0)}),this.room&&(this.setupRoomListeners(),this.startGameEngine("PreloadScene",{room:this.room})),this.characterPopup=new S(P,a=>{this.room&&this.room.send("updateHair",{hairId:a}),localStorage.setItem("player_hair_id",a.toString())},()=>{}),this.updateAll(),this.updateUILayout(),this.room){this.room.state.listen("isPreparing",o=>{o&&d.showWaiting(r.t("host_lobby.preparing")||"PREPARING GAME...",0)});let a=!1,i=0;this.room.state.listen("countdown",(o,n)=>{if(o>0){a||(a=!0,m.getInstance().stopBGM(),m.getInstance().playCountdownSFX());const s=Math.max(0,Math.ceil((i-Date.now())/1e3));if((Math.abs(s-o)>1||!this.countdownInterval)&&(i=Date.now()+o*1e3),!this.countdownInterval){const l=()=>{const c=i-Date.now(),p=Math.max(0,Math.ceil(c/1e3));p>0?(d.ensureClosed(),d.setCountdownText(p.toString())):(this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null),d.setCountdownText("GO!"))};l(),this.countdownInterval=setInterval(l,100)}}else o===0&&(n||0)>0&&(this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null),d.setCountdownText("GO!"),setTimeout(()=>{this.handleGameStart()},100))}),this.room.state.listen("isGameStarted",o=>{o&&this.handleGameStart()}),b.requireLandscape()}}handleGameStart(){this.isGameStarting||(this.isGameStarting=!0,this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null),d.ensureClosed(),this.waitingUI&&this.waitingUI.classList.add("hidden"),y.navigate("/game"),this.startGameEngine("GameScene",{room:this.room}))}setupPlayerUI(){if(!this.waitingUI)return;const t="player-waiting-room-styles";if(!document.getElementById(t)){const o=document.createElement("style");o.id=t,o.innerHTML=`
                .player-content-box {
                    background: #2d5a27;
                    border: none;
        ...
        `,document.body.appendChild(t),(a=document.getElementById("host-left-ok-btn"))==null||a.addEventListener("click",()=>{t.remove(),this.leaveRoom()})}leaveRoom(){if(this.isManuallyLeaving=!0,console.log("[PlayerLobby] 🚪 Leaving room manually..."),localStorage.removeItem("currentRoomId"),localStorage.removeItem("currentSessionId"),localStorage.removeItem("currentReconnectionToken"),localStorage.removeItem("pendingJoinRoomCode"),this.room){const t=this.room;this.room=null;try{t.connection&&t.connection.isOpen&&(t.send("manualPlayerLeave"),console.log("[PlayerLobby] ✅ manualPlayerLeave signal sent"))}catch(e){console.warn("[PlayerLobby] ⚠️ manualPlayerLeave signal failed:",e)}try{t.connection&&t.connection.isOpen&&(t.leave(!0),console.log("[PlayerLobby] ✅ Room.leave(true) called"))}catch{}setTimeout(()=>{this.navigateToLobby()},100)}else this.navigateToLobby()}navigateToLobby(){console.log("[PlayerLobby] 🏠 Navigating back to lobby..."),this.waitingUI&&this.waitingUI.classList.add("hidden");const t=document.getElementById("waiting-ui");t&&t.classList.add("hidden"),this.waitingSpawnerInterval&&(clearInterval(this.waitingSpawnerInterval),this.waitingSpawnerInterval=null),this.countdownInterval&&(clearInterval(this.countdownInterval),this.countdownInterval=null),b.disable(),y.replace("/"),this.startManager("LobbyManager",{didExit:!0})}updateAll(){var e;if(!((e=this.room)!=null&&e.state))return;this.updatePlayerGrid();const t=this.room.state.players.get(this.mySessionId);t&&this.updateCharacterPreview(t.hairId||0)}updateUILayout(){var a;if(!((a=this.room)!=null&&a.state))return;const t=new Map;this.room.state.players.forEach((i,o)=>{if(i.isHost)return;const n=i.userId||o;t.has(n)||t.set(n,i)});const e=t.size;this.playerCountEl&&(this.playerCountEl.innerText=e.toString())}updateRoomList(){if(!this.roomListEl)return;const t=this.room.state.players.get(this.mySessionId),e=t?t.subRoomId:"";let a="";this.room.state.subRooms.forEach(i=>{const o=i.id===e,n=i.playerIds.length,s=n>=i.capacity,l=o?"border-primary bg-primary/10 shadow-[0_0_15px_rgba(0,255,85,0.1)]":"border-white/10 bg-black/40 hover:border-white/30",c=o?"text-primary":"text-white",p=s?"bg-white/10 text-white/30 cursor-not-allowed border-gray-600":o?"bg-primary text-black font-bold pixel-btn-green border-black":"bg-secondary text-black font-bold pixel-btn-blue border-black hover:brightness-110",_=o?r.t("host_lobby.joined"):s?r.t("host_lobby.full"):r.t("host_lobby.join"),k=this.isHost||s||o?"":`onclick="window.switchRoom('${i.id}')"`,E=this.isHost?"invisible":"";let h="",u=0;i.playerIds.forEach(f=>{var v,w,I;const x=(I=(w=(v=this.room)==null?void 0:v.state)==null?void 0:w.players)==null?void 0:I.get(f);if(x){const L=f===this.mySessionId?"text-primary":"text-white/70";h+=`
                        <div class="flex items-center gap-2 ${L} text-[10px] font-bold uppercase truncate">
                            ...
                            ${x.name}
                        </div>`,u++}}),u===0&&(h=`<span class="text-[10px] text-white/30 italic pl-1">${r.t("host_lobby.empty")}</span>`),a+=`
                <div class="w-full max-w-[320px] border-2 ${l} p-4 rounded-xl transition-all duration-300 relative group">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-sm font-bold uppercase ${c} font-['Press_Start_2P'] tracking-tight">${i.id}</span>
                    ...
                    <button ${k} class="w-full py-3 text-xs uppercase rounded-lg border-b-4 active:border-b-0 active:translate-y-1 transition-all ${p} font-['Press_Start_2P'] tracking-wide ${E}">
                        ${_}
                    </button>
                </div>
            `}),this.roomListEl.innerHTML=a,window.switchRoom=i=>{this.room.send("switchRoom",{roomId:i})}}updatePlayerGrid(){var i;if(!this.playerGridEl||!((i=this.room)!=null&&i.state))return;const t=new Map;this.room.state.players.forEach((o,n)=>{if(o.isHost)return;const s=o.userId||n;(!t.get(s)||n===this.mySessionId||this.isHost)&&t.set(s,{sessionId:n,name:o.name,hairId:o.hairId})});const e=Array.from(t.values());e.sort((o,n)=>o.sessionId===this.mySessionId?-1:n.sessionId===this.mySessionId?1:0),this.updateUILayout();let a="";e.forEach(o=>{const n=o.sessionId===this.mySessionId,s=n?"player-card-standard player-card-active":"player-card-standard",l=n?`<div class="absolute -bottom-3 left-1/2 -translate-x-1/2 pill-you">${r.t("player_lobby.you")}</div>`:"";a+=`
                    ...
                    <div style="width: 80px; height: 80px; display: flex; align-items: center; justify-content: center; overflow: visible; margin-top: 5px;">
                         ...
                         ">
                            ...
                            <div style="
                                ...
                                image-rendering: pixelated;
                            "></div>
                            ${(()=>{const c=R(o.hairId||0);return o.hairId>0&&c?`
                                        <div style="
                                            position: absolute; inset: 0;
