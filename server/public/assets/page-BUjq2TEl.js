const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/QuizData-CZh1xzyf.js","assets/index-Ct-ulYoa.js"])))=>i.map(i=>d[i]);
import{a as T,G as w,i as s,T as g,O as I,l as $,R as S,_ as L,c as B,L as E}from"./index-Ct-ulYoa.js";import{g as C}from"./xid-2V5ChoND.js";import{o as z}from"./statsSession-RS9odABG.js";class R{static async createRoom(t,i){const{difficulty:o,questionCount:r,timer:l,quiz:n}=i;let c="map_newest_easy_nomor1.tmj";o==="sedang"&&(c="map_baru2.tmj"),o==="sulit"&&(c="map_baru3.tmj");const m=r===5?10:20,p=this.generateRoomCode(),e=T.getStoredProfile();if(!(e!=null&&e.id))throw new Error("HOST_LOGIN_REQUIRED");const a=e.id;let h=[...n.questions||[]];h.sort(()=>Math.random()-.5),h=h.slice(0,r);const b=C(),x={roomCode:p,sessionId:b,difficulty:o,subject:n.category?n.category.toLowerCase():"umum",quizId:n.id,quizTitle:n.title,questions:h,map:c,questionCount:r,enemyCount:m,timer:l,isHost:!0,hostId:a,quizDetail:{title:n.title,category:n.category,language:n.language||"id",description:n.description,creator_avatar:n.creator_avatar||null,creator_username:n.creator_username||"kizuko"}};try{localStorage.setItem("currentRoomOptions",JSON.stringify(x));const d=await t.create("game_room",x);return console.log("Room created via RoomService!",d),localStorage.setItem("currentRoomId",d.id),localStorage.setItem("currentSessionId",d.sessionId),localStorage.setItem("currentReconnectionToken",d.reconnectionToken),localStorage.setItem("supabaseSessionId",b),{room:d,options:x}}catch(d){throw console.error("RoomService Flow Error:",d),d}}static generateRoomCode(){return Math.floor(1e5+Math.random()*9e5).toString()}}class k{static getGlobalStyles(){return`
        ...
        `}static generateHTML(t){const i=[t[0],t[1],t[2]],o=t,r=e=>{const a=Math.floor(e/1e3),h=Math.floor(a/60),f=a%60;return`${h}:${f.toString().padStart(2,"0")}`},l=e=>{if(!e)return"?";const a=e.trim();return a.length<=1?a.toUpperCase():a.substring(0,1).toUpperCase()+a.substring(1,2).toLowerCase()},n=e=>e&&(e.includes("googleusercontent.com")?e.replace(/=s\d+(-c)?/,"=s384-c"):e),m=[i[1],i[0],i[2]].map(e=>{if(!e)return'<div class="w-[100px] md:w-[150px]"></div>';const a=e.rank,h=a===1,f=a===2;let b="#cd7f32",x="#8B4513",d="rgba(205,127,50,0.5)",u="h-28 md:h-36",v="w-12 h-12 md:w-16 md:h-16",y="text-5xl md:text-7xl";return h?(b="#ffcc00",x="#B8860B",d="rgba(255,204,0,0.6)",u="h-44 md:h-56",v="w-16 h-16 md:w-24 md:h-24",y="text-7xl md:text-9xl"):f&&(b="#c0c0c0",x="#708090",d="rgba(192,192,192,0.5)",u="h-36 md:h-44",v="w-14 h-14 md:w-20 md:h-20",y="text-5xl md:text-7xl"),`
