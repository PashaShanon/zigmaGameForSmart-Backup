const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/QuizData-CTr8p6ML.js","assets/index-wtlkDWDF.js"])))=>i.map(i=>d[i]);
import{a as L,G as w,i as s,T as f,O as _,l as T,R as S,_ as $,c as B,L as O}from"./index-wtlkDWDF.js";import{g as C}from"./xid-2V5ChoND.js";class E{static async createRoom(e,i){const{difficulty:o,questionCount:r,timer:d,quiz:n}=i;let h="map_newest_easy_nomor1.tmj";o==="sedang"&&(h="map_baru2.tmj"),o==="sulit"&&(h="map_baru3.tmj");const b=r===5?10:20,g=this.generateRoomCode(),t=L.getStoredProfile(),a=t?t.id:null;let m=[...n.questions||[]];m.sort(()=>Math.random()-.5),m=m.slice(0,r);const l=C(),x={roomCode:g,sessionId:l,difficulty:o,subject:n.category?n.category.toLowerCase():"umum",quizId:n.id,quizTitle:n.title,questions:m,map:h,questionCount:r,enemyCount:b,timer:d,isHost:!0,hostId:a,quizDetail:{title:n.title,category:n.category,language:n.language||"id",description:n.description,creator_avatar:n.creator_avatar||null,creator_username:n.creator_username||"kizuko"}};try{localStorage.setItem("currentRoomOptions",JSON.stringify(x));const c=await e.create("game_room",x);return console.log("Room created via RoomService!",c),localStorage.setItem("currentRoomId",c.id),localStorage.setItem("currentSessionId",c.sessionId),localStorage.setItem("currentReconnectionToken",c.reconnectionToken),localStorage.setItem("supabaseSessionId",l),{room:c,options:x}}catch(c){throw console.error("RoomService Flow Error:",c),c}}static generateRoomCode(){return Math.floor(1e5+Math.random()*9e5).toString()}}class k{static getGlobalStyles(){return`
        ...
        `}static generateHTML(e){const i=[e[0],e[1],e[2]],o=e,r=t=>{const a=Math.floor(t/1e3),m=Math.floor(a/60),p=a%60;return`${m}:${p.toString().padStart(2,"0")}`},d=t=>{if(!t)return"?";const a=t.trim();return a.length<=1?a.toUpperCase():a.substring(0,1).toUpperCase()+a.substring(1,2).toLowerCase()},n=t=>t&&(t.includes("googleusercontent.com")?t.replace(/=s\d+(-c)?/,"=s384-c"):t),b=[i[1],i[0],i[2]].map(t=>{if(!t)return'<div class="w-[100px] md:w-[150px]"></div>';const a=t.rank,m=a===1,p=a===2;let l="#cd7f32",x="#8B4513",c="rgba(205,127,50,0.5)",u="h-28 md:h-36",v="w-12 h-12 md:w-16 md:h-16",y="text-5xl md:text-7xl";return m?(l="#ffcc00",x="#B8860B",c="rgba(255,204,0,0.6)",u="h-44 md:h-56",v="w-16 h-16 md:w-24 md:h-24",y="text-7xl md:text-9xl"):p&&(l="#c0c0c0",x="#708090",c="rgba(192,192,192,0.5)",u="h-36 md:h-44",v="w-14 h-14 md:w-20 md:h-20",y="text-5xl md:text-7xl"),`
                <div class="flex flex-col items-center relative z-20 group">
                    
                    <!-- 1. PROFILE (TOP) -->
                    <div class="${v} podium-avatar rounded-full border-[2.5px] md:border-[4px] flex items-center justify-center font-bold relative mb-3 shadow-2xl transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_30px_${c}]" 
