const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/QuizData-E7nwSj9_.js","assets/index-BILLolT4.js"])))=>i.map(i=>d[i]);
import{a as k,s as $,S as B,G as v,i as s,T as f,O as _,l as L,R as I,_ as E,b as z,L as R}from"./index-BILLolT4.js";class T{static async createRoom(e,i){const{difficulty:o,questionCount:r,timer:l,quiz:a}=i;let b="map_newest_easy_nomor1.tmj";o==="sedang"&&(b="map_baru2.tmj"),o==="sulit"&&(b="map_baru3.tmj");const x=r===5?10:20,g=this.generateRoomCode(),t=k.getStoredProfile(),n=t?t.id:null;let c=[...a.questions||[]];c.sort(()=>Math.random()-.5),c=c.slice(0,r);try{const{data:d,error:m}=await $.from(B).insert({game_pin:g,quiz_id:a.id,status:"waiting",question_limit:r,total_time_minutes:l/60,difficulty:o,host_id:n,created_at:new Date().toISOString(),current_questions:c}).select().single();if(m)throw console.error("Supabase Session Error:",m),new Error("Failed to create game session.");console.log("Session Created in Supabase B:",d);const h={roomCode:g,sessionId:d.id,difficulty:o,subject:a.category.toLowerCase(),quizId:a.id,quizTitle:a.title,questions:c,map:b,questionCount:r,enemyCount:x,timer:l,isHost:!0,hostId:n,quizDetail:{title:a.title,category:a.category,language:a.language||"id",description:a.description,creator_avatar:a.creator_avatar||null,creator_username:a.creator_username||"kizuko"}};localStorage.setItem("currentRoomOptions",JSON.stringify(h));const p=await e.create("game_room",h);return console.log("Room created via RoomService!",p),localStorage.setItem("currentRoomId",p.id),localStorage.setItem("currentSessionId",p.sessionId),localStorage.setItem("currentReconnectionToken",p.reconnectionToken),localStorage.setItem("supabaseSessionId",d.id),{room:p,options:h}}catch(d){throw console.error("RoomService Flow Error:",d),d}}static generateRoomCode(){return Math.floor(1e5+Math.random()*9e5).toString()}}class S{static getGlobalStyles(){return`
