import{i as e,T as u,O as x,R as f,G as b}from"./index-Ct-ulYoa.js";import{o as v}from"./statsSession-RS9odABG.js";class k{constructor(){this.rankings=[],this.mySessionId="",this.roomId="",this.supabaseSessionId="",this.handleLangChange=()=>{const t=document.getElementById("txt-pr-rank");t&&(t.innerText=e.t("player_result.rank"));const i=document.getElementById("txt-pr-score");i&&(i.innerText=e.t("player_result.score"));const l=document.getElementById("txt-pr-correct");l&&(l.innerText=e.t("player_result.correct"));const a=document.getElementById("txt-pr-time");a&&(a.innerText=e.t("player_result.time"));const s=document.getElementById("lb-home-btn");s&&(s.title=e.t("player_result.title_home"));const n=document.getElementById("lb-stats-btn");n&&(n.title=e.t("player_result.title_stats"));const d=document.getElementById("txt-pr-home");d&&(d.innerText=e.t("player_result.home"));const p=document.getElementById("txt-pr-stats");p&&(p.innerText=e.t("player_result.stats"))}}start(t){var n,d,p,h,o,c,g;document.title="Result | Zigma",u.ensureClosed(),x.requirePortrait(e.t("player_result.portrait_req_title"),e.t("player_result.portrait_req_desc")),window.addEventListener("languageChanged",this.handleLangChange);let i=(t==null?void 0:t.leaderboardData)||[];this.room=t==null?void 0:t.room;let l=this.room?this.room.sessionId:"",a=this.room?this.room.id:"";if(this.supabaseSessionId=localStorage.getItem("supabaseSessionId")||((d=(n=this.room)==null?void 0:n.metadata)==null?void 0:d.sessionId)||"",i.length>0&&l&&a){this.rankings=i,this.mySessionId=l,this.roomId=a;const r=((h=(p=this.room)==null?void 0:p.state)==null?void 0:h.questionLimit)||"5",m=((g=(c=(o=this.room)==null?void 0:o.state)==null?void 0:c.questions)==null?void 0:g.length)||5,y=r==="all"?m:parseInt(r);sessionStorage.setItem("playerResultState",JSON.stringify({rankings:this.rankings,mySessionId:this.mySessionId,roomId:this.roomId,supabaseSessionId:this.supabaseSessionId,questionTotal:y}))}else{const r=sessionStorage.getItem("playerResultState");if(r){const m=JSON.parse(r);this.rankings=m.rankings,this.mySessionId=m.mySessionId,this.roomId=m.roomId,m.supabaseSessionId&&(this.supabaseSessionId=m.supabaseSessionId)}}const s=document.getElementById("result-ui");if(s&&s.remove(),this.container=document.createElement("div"),this.container.id="result-ui",this.container.style.position="absolute",this.container.style.top="0",this.container.style.left="0",this.container.style.width="100%",this.container.style.height="100%",this.container.style.zIndex="1000",this.container.style.pointerEvents="none",document.body.appendChild(this.container),!document.getElementById("result-styles")){const r=document.createElement("style");r.id="result-styles",r.innerHTML=this.getStyles(),document.head.appendChild(r)}this.renderIndividualResult(),this.room&&this.room.onMessage("gameEnded",r=>{this.rankings=r.rankings,sessionStorage.setItem("playerResultState",JSON.stringify({rankings:this.rankings,mySessionId:this.mySessionId,roomId:this.roomId,supabaseSessionId:this.supabaseSessionId})),this.renderIndividualResult()}),setTimeout(()=>{u.open()},100)}getStyles(){return`
        ...
        `}renderIndividualResult(){var d,p,h;f.navigate("/player/result"),this.container.innerHTML="";const t=this.rankings.find(o=>o.sessionId===this.mySessionId)||this.rankings[0];if(!t)return;const i=o=>{const c=Math.floor(o/1e3),g=Math.floor(c/60),r=c%60;return`${g.toString().padStart(2,"0")}:${r.toString().padStart(2,"0")}`},l=o=>o&&(o.includes("googleusercontent.com")?o.replace(/=s\d+(-c)?/,"=s384-c"):o),a=o=>{if(!o)return"?";const c=o.trim();return c.length<=1?c.toUpperCase():c.substring(0,1).toUpperCase()+c.substring(1,2).toLowerCase()},s=sessionStorage.getItem("playerResultState"),n=s&&JSON.parse(s).questionTotal||5;this.getCharacterVisuals(t),this.container.innerHTML=`
            ${b.getHTML("result")}
        ...
        `,b.startCharacterSpawner("result"),this.attachListeners()}getCharacterVisuals(t){const i="/assets/characters/Human/IDLE",l=`background-image: url('${i}/base_idle_strip9.png'); background-size: 864px 64px;`,a=`background-image: url('${i}/tools_idle_strip9.png'); background-size: 864px 64px;`;let s="";if(t.hairId&&t.hairId>0){const d={1:"bowlhair",2:"curlyhair",3:"longhair",4:"mophair",5:"shorthair",6:"spikeyhair"}[t.hairId];d&&(s=`background-image: url('${i}/${d}_idle_strip9.png'); background-size: 864px 64px;`)}return{base:l,tools:a,hair:s}}attachListeners(){setTimeout(()=>{const t=document.getElementById("lb-home-btn"),i=document.getElementById("lb-stats-btn");t&&(t.onclick=()=>{if(this.room){try{this.room.send("manualPlayerLeave")}catch(n){console.warn("manualPlayerLeave failed:",n)}const s=this.room;this.room=null,setTimeout(()=>{try{s.connection&&s.connection.isOpen&&s.leave(!0)}catch{}u.transitionTo(()=>{this.cleanup(),window.location.href="/"})},100)}else u.transitionTo(()=>{this.cleanup(),window.location.href="/"})});const l=document.getElementById("lb-home-btn-mobile");l&&(l.onclick=()=>{if(this.room){try{this.room.send("manualPlayerLeave")}catch(n){console.warn("manualPlayerLeave failed:",n)}const s=this.room;this.room=null,setTimeout(()=>{try{s.connection&&s.connection.isOpen&&s.leave(!0)}catch{}u.transitionTo(()=>{this.cleanup(),window.location.href="/"})},100)}else u.transitionTo(()=>{this.cleanup(),window.location.href="/"})}),i&&(i.onclick=()=>this.openStats());const a=document.getElementById("lb-stats-btn-mobile");a&&(a.onclick=()=>this.openStats())},50)}openStats(){const t={metadata:{sessionId:this.supabaseSessionId||localStorage.getItem("supabaseSessionId")||void 0}};v(t,()=>alert(e.t("player_result.no_session")))}cleanup(){b.stopCharacterSpawner("result"),this.container&&this.container.remove();const t=document.getElementById("result-styles");t&&t.remove(),x.disable(),window.removeEventListener("languageChanged",this.handleLangChange)}}export{k as ResultManager};
