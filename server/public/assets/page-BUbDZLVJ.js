import{_ as m,O as h,R as b,A as d,i as s,C as E,H as k,T as p,g as S}from"./index-BILLolT4.js";class M{constructor(){this.isHost=!1,this.mySessionId="",this.isGameStarting=!1,this.waitingUI=null,this.playerGridEl=null,this.playerCountEl=null,this.startBtn=null,this.waitingMsg=null,this.nameInput=null,this.roomListEl=null,this.hostIndicatorEl=null,this.backBtn=null,this.characterPopup=null,this.characterPreviewEl=null,this.waitingSpawnerInterval=null}async init(t){t.room&&(this.room=t.room,this.mySessionId=this.room.sessionId),this.isHost=t.isHost!==void 0?t.isHost:!1,t.isRestore&&!this.room&&t.client&&await this.restoreRoom(t.client),this.start()}async restoreRoom(t){const i=localStorage.getItem("currentReconnectionToken");if(!i){console.warn("Cannot restore player room: No reconnection token saved."),this.cleanupAndGoLobby();return}try{console.log("Player reconnecting with token..."),this.room=await t.reconnect(i),console.log("Player reconnected!",this.room),this.mySessionId=this.room.sessionId,localStorage.setItem("currentReconnectionToken",this.room.reconnectionToken),this.setupRoomListeners()}catch(a){console.warn("Player reconnection failed:",a),localStorage.removeItem("currentRoomId"),localStorage.removeItem("currentSessionId"),localStorage.removeItem("currentReconnectionToken"),this.cleanupAndGoLobby()}}startGameEngine(t,i){m(()=>import("./index-BILLolT4.js").then(a=>a.e),[]).then(a=>{a.initializeGame(t,i)}).catch(a=>{console.error("Failed to load game engine:",a),window.location.href="/"})}startManager(t,i){t==="LobbyManager"&&m(()=>import("./index-BILLolT4.js").then(a=>a.p),[]).then(a=>{new a.LobbyManager().init(i)})}cleanupAndGoLobby(){var i;this.waitingUI&&this.waitingUI.classList.add("hidden");const t=document.getElementById("waiting-ui");t&&t.classList.add("hidden"),(i=document.getElementById("exit-confirm-modal"))==null||i.remove(),h.disable(),b.navigate("/"),this.startManager("LobbyManager")}setupRoomListeners(){this.room.state.listen("hostId",t=>{t===this.mySessionId&&this.startGameEngine("HostWaitingRoomScene",{room:this.room,isHost:!0}),this.updateUILayout()}),this.room.state.listen("isMusicEnabled",t=>{console.log(`[PlayerLobby] 🏠 Room Music Enabled: ${t}`),d.getInstance().setRoomMute(!t)}),this.room.state.isMusicEnabled!==void 0&&d.getInstance().setRoomMute(!this.room.state.isMusicEnabled),this.room.state.players.onAdd((t,i)=>{this.updateAll(),t.listen("name",()=>this.updateAll()),t.listen("hairId",()=>this.updateAll())}),this.room.state.players.onRemove(()=>this.updateAll()),this.room.onMessage("gameStarted",()=>{this.handleGameStart()}),this.room.onMessage("kicked",t=>{this.leaveRoom()}),this.room.onMessage("hostLeft",()=>{this.showHostLeftModal()})}start(){window.addEventListener("languageChanged",()=>{this.backBtn&&(this.backBtn.innerText=s.t("player_lobby.exit"));const a=document.getElementById("player-choose-char-btn");a&&(a.innerText=s.t("player_lobby.choose_character"));const e=document.getElementById("exit-confirm-title");e&&(e.innerHTML=s.t("player_lobby.dialog_exit_title"));const o=document.getElementById("exit-confirm-desc");o&&(o.innerHTML=s.t("player_lobby.dialog_exit_desc"));const l=document.getElementById("exit-cancel-btn");l&&(l.innerText=s.t("player_lobby.dialog_exit_cancel"));const r=document.getElementById("exit-confirm-btn");r&&(r.innerText=s.t("player_lobby.dialog_exit_confirm")),this.updatePlayerGrid()}),this.waitingUI=document.getElementById("waiting-ui");const t=document.getElementById("lobby-ui");t&&t.classList.add("hidden"),this.waitingUI&&(this.waitingUI.classList.remove("hidden"),this.setupPlayerUI()),this.playerGridEl=document.getElementById("player-grid"),this.playerCountEl=document.getElementById("player-count-value"),this.nameInput=document.getElementById("header-player-name"),this.backBtn=document.getElementById("player-back-btn"),this.backBtn&&(this.backBtn.onclick=()=>{this.showExitConfirm()});const i=document.getElementById("player-choose-char-btn");i&&(i.onclick=()=>{var e;const a=this.room.state.players.get(this.mySessionId);(e=this.characterPopup)==null||e.show((a==null?void 0:a.hairId)||0)}),this.room&&this.setupRoomListeners(),this.characterPopup=new E(k,a=>{this.room&&this.room.send("updateHair",{hairId:a})},()=>{}),this.updateAll(),this.updateUILayout(),this.room&&(this.room.state.listen("countdown",(a,e)=>{a>0?(p.ensureClosed(),p.setCountdownText(a.toString()),d.getInstance().stopBGM(),d.getInstance().playCountdownSFX(),this.handleGameStart()):a===0&&(e||0)>0&&p.setCountdownText("GO!")}),this.room.state.listen("isGameStarted",a=>{a&&this.handleGameStart()}),h.requireLandscape())}handleGameStart(){this.isGameStarting||(this.isGameStarting=!0,p.ensureClosed(),this.waitingUI&&this.waitingUI.classList.add("hidden"),b.navigate("/game"),this.startGameEngine("GameScene",{room:this.room}))}setupPlayerUI(){if(!this.waitingUI)return;const t="player-waiting-room-styles";if(!document.getElementById(t)){const o=document.createElement("style");o.id=t,o.innerHTML=`
        ...
        `,document.body.appendChild(t),(a=document.getElementById("host-left-ok-btn"))==null||a.addEventListener("click",()=>{t.remove(),this.leaveRoom()})}leaveRoom(){this.room&&this.room.leave(),localStorage.removeItem("currentRoomId"),localStorage.removeItem("currentSessionId"),localStorage.removeItem("currentReconnectionToken"),localStorage.removeItem("pendingJoinRoomCode"),this.waitingUI&&this.waitingUI.classList.add("hidden"),h.disable();const t=document.getElementById("lobby-ui");t&&t.classList.remove("hidden"),b.replace("/"),this.startManager("LobbyManager",{didExit:!0})}updateAll(){this.updatePlayerGrid();const t=this.room.state.players.get(this.mySessionId);t&&this.updateCharacterPreview(t.hairId||0)}updateUILayout(){let t=0;this.room.state.players.forEach(i=>{i.isHost||t++}),this.playerCountEl&&(this.playerCountEl.innerText=t.toString())}updateRoomList(){if(!this.roomListEl)return;const t=this.room.state.players.get(this.mySessionId),i=t?t.subRoomId:"";let a="";this.room.state.subRooms.forEach(e=>{const o=e.id===i,l=e.playerIds.length,r=l>=e.capacity,n=o?"border-primary bg-primary/10 shadow-[0_0_15px_rgba(0,255,85,0.1)]":"border-white/10 bg-black/40 hover:border-white/30",f=o?"text-primary":"text-white",u=r?"bg-white/10 text-white/30 cursor-not-allowed border-gray-600":o?"bg-primary text-black font-bold pixel-btn-green border-black":"bg-secondary text-black font-bold pixel-btn-blue border-black hover:brightness-110",v=o?s.t("host_lobby.joined"):r?s.t("host_lobby.full"):s.t("host_lobby.join"),w=this.isHost||r||o?"":`onclick="window.switchRoom('${e.id}')"`,I=this.isHost?"invisible":"";let c="",g=0;e.playerIds.forEach(y=>{const x=this.room.state.players.get(y);if(x){const _=y===this.mySessionId?"text-primary":"text-white/70";c+=`
                        <div class="flex items-center gap-2 ${_} text-[10px] font-bold uppercase truncate">
                            ...
                            ${x.name}
                        </div>`,g++}}),g===0&&(c=`<span class="text-[10px] text-white/30 italic pl-1">${s.t("host_lobby.empty")}</span>`),a+=`
                <div class="w-full max-w-[320px] border-2 ${n} p-4 rounded-xl transition-all duration-300 relative group">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-sm font-bold uppercase ${f} font-['Press_Start_2P'] tracking-tight">${e.id}</span>
                    ...
                    <button ${w} class="w-full py-3 text-xs uppercase rounded-lg border-b-4 active:border-b-0 active:translate-y-1 transition-all ${u} font-['Press_Start_2P'] tracking-wide ${I}">
                        ${v}
                    </button>
                </div>
            `}),this.roomListEl.innerHTML=a,window.switchRoom=e=>{this.room.send("switchRoom",{roomId:e})}}updatePlayerGrid(){if(!this.playerGridEl)return;const t=new Map;this.room.state.players.forEach((e,o)=>{!e.isHost&&!t.has(o)&&t.set(o,{sessionId:o,name:e.name,hairId:e.hairId,isHost:e.isHost})});const i=Array.from(t.values());i.sort((e,o)=>e.sessionId===this.mySessionId?-1:o.sessionId===this.mySessionId?1:0),this.updateUILayout();let a="";i.forEach(e=>{const o=e.sessionId===this.mySessionId,l=o?"player-card-standard player-card-active":"player-card-standard",r=o?`<div class="absolute -bottom-3 left-1/2 -translate-x-1/2 pill-you">${s.t("player_lobby.you")}</div>`:"";a+=`
                <div class="${l} player-card-wrapper">
                    ...
                    <div style="width: 80px; height: 80px; display: flex; align-items: center; justify-content: center; overflow: visible; margin-top: 5px;">
                         ...
                         ">
                            ...
                            <div style="
                                ...
                                image-rendering: pixelated;
                            "></div>
                            ${(()=>{const n=S(e.hairId||0);return e.hairId>0&&n?`
                                        <div style="
                                            position: absolute; inset: 0;
                    ...
                    <div class="player-name-container" style="text-align: center; width: 100%; margin-top: 4px; padding: 0 4px;">
                        <span style="font-size: 14px; color: #FFFFFF; font-family: 'Press Start 2P', cursive; text-transform: uppercase; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; width: 100%; ${o?"text-shadow: 0 0 8px rgba(255, 255, 255, 0.4);":""}">
